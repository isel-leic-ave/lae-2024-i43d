package pt.isel

import kotlin.coroutines.Continuation
import kotlin.coroutines.EmptyCoroutineContext
import kotlin.coroutines.intrinsics.COROUTINE_SUSPENDED
import kotlin.coroutines.resume

fun main() {
    val res = fetchManySuspendLazy(
        "https://github.com",
        "https://stackoverflow.com/",
        "https://developer.mozilla.org"
    )
    val iter = res.iterator()
    iter.next().let { println(it
        .split("<title>")[1]
        .split("</title")[0]) }
    iter.next().let { println(it
        .split("<title>")[1]
        .split("</title")[0]) }
}

fun fetchManySuspendLazy(url1: String, url2: String, url3: String) : Sequence<String> {
    return object:Sequence<String> {
        override fun iterator(): Iterator<String> {
            return FetchManySuspendIterator(url1, url2, url3)
        }

    }
}

class FetchManySuspendIterator(
    val url1: String,
    val url2: String,
    val url3: String
) : Iterator<String>, Continuation<Unit> {
    var resFinal: Result<Unit>? = null
    var label = 0
    lateinit var nextItem: String
    var nextContinuation: Continuation<Unit>? = null

    val blockCps = ::block as ((Continuation<Unit>) -> Any)

    suspend fun block() {
        val body1 = fetch(url1)
        yield(body1)
        val body2 = fetch(url2)
        yield(body2)
        val body3 = fetch(url3)
        yield(body3)
    }

    suspend fun yield(item: String) = yieldCpsHandle(item)

    val yieldCpsHandle = ::yieldCps as (suspend (String) -> Unit)

    fun yieldCps(item: String, cont: Continuation<Unit>) : Any {
        nextItem = item
        nextContinuation = cont
        return COROUTINE_SUSPENDED
    }
    override fun hasNext() = resFinal == null
    override fun next(): String {
        if(!hasNext()) throw NoSuchElementException()
        nextContinuation
            ?.resume(Unit)
            ?: blockCps(this)
        return nextItem
    }

    override val context = EmptyCoroutineContext
    override fun resumeWith(result: Result<Unit>) {
        resFinal = result
    }

}